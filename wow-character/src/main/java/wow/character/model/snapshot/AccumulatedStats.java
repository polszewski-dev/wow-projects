...
import wow.commons.model.attribute.primitive.AttributeTarget;
...
public abstract class AccumulatedStats {
	...
	public void accumulateAttributes(List<PrimitiveAttribute> attributes, double scaleFactor) {
	...
	private static boolean isMatchingTarget(PrimitiveAttribute attribute) {
		return attribute.id().getTarget() != AttributeTarget.PET;
	}

	protected abstract void accumulateAttribute(PrimitiveAttribute attribute, double scaleFactor);
	...
	protected double getAccumulatedValue(PrimitiveAttributeId attributeId, BaseStatsSnapshot baseStats) {
